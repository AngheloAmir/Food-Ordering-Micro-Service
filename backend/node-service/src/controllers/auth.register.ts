...
export default async function AuthCreateNewAccount(req: Request, res: Response) {
    ...
    try {
        const { error: createError } = await createSupabaseAdmin().auth.admin.createUser({
        ...
        const { data: loginData, error: loginError } = await createSupabaseAdmin().auth.signInWithPassword({
            email,
            password,
        });
